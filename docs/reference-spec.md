...
- **Testimonials `#testimonials`** min-h 100svh-60, 4 cols divide-x: [col 1: rows-2: eyebrow + prev/next (h-60, progress bar h-4 scaleX = (i+1)/n) · portrait (109% zoom, clip inset)] [cols 2-3: quote mark svg + headline-10 text; bottom: name + role mono 50% · odometer `01 - 09`] [col 4 rows-2: dark sticky Position/Company · Dither].
- **Team `#team`** rows-2 100svh-60: [h2 `Team` sticky · `Small team. / High Signal.` headline-10 bottom] [eyebrow `WHO YOU'LL ACTUALLY WORK WITH` + prev/next · paragraph]. Then horizontal snap slider `-mt-[40vh] -mb-[20vh] py-[20vh]`, cards basis (100%-3px)/4 aspect .8, each with random vertical offset (20/55/35/70/40/25/60/45 %), photo (cursor RGB-split shader), mint name button bottom (body-20, arrow), role tag top-right. Click opens modal.
...
- Entrance: black overlay z-3; logo opacity 0→1, scale .4→1 (inner .3→1) over ~2s from `load`; hold; wipe
  `clip-path: inset(0) → inset(0 100% 0 0)` (~1s ease-in-out); header logo/time/nav reveal; text lines rise.
- Text reveal (`AnimatedText`): split into lines, each line wrapped in a `clip-path` mask, `translateY(100%)→0`,
  opacity 0→1, staggered; triggers on viewport entry, once. `[measured]` from DOM (`data-mask`, `data-line`).
